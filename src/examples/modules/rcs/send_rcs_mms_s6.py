...
if __name__ == '__main__':
    ...
    sample3 = res['fileId']
    res = storage.upload_rcs_image('images/sample4.png').json()
    sample4 = res['fileId']
    res = storage.upload_rcs_image('images/sample5.png').json()
    sample5 = res['fileId']
    res = storage.upload_rcs_image('images/sample6.png').json()
    sample6 = res['fileId']

    # 카드 6개 발송 예제(각 카드별 버튼 2개까지 가능)
    data = {
        'messages': [
            {
                ...
                'from': '029302266',
                'subject': 'Sample 1',
                'text': 'MMS S6 타입 발송 예제입니다.',
                'imageId': sample1,
                'rcsOptions': {
                    'mmsType': 'S6',  # S3 ~ S6 (총 이미지 1M를 넘을 수 없음)
